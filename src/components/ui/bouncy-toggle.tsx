...
interface PremiumToggleProps {
  /** Controlled value. When provided, the toggle reflects this prop. */
  checked?: boolean
  /** Uncontrolled initial value (used only when `checked` is undefined). */
...
export function PremiumToggle({ checked, defaultChecked = false, onChange, label, disabled }: PremiumToggleProps) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        className={cn(
          "group relative h-8 w-14 shrink-0 rounded-full p-1 transition-all duration-500 ease-out",
